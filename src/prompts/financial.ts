...
export function buildAnalysisPrompt(stockCode: string, quarters: string[]): string {
  ...
  const stockHint = stockCode
    ? `股票代碼為 ${stockCode}。`
    : `請從財報內容自動辨識股票代碼（純數字，如 2330，不含 .TW）。`

  return `${orderNote}

${stockHint}請分析此財務報表，嚴格以下列 JSON 格式回傳（不含任何 markdown 包裝或說明文字）：

{
  "company": "公司完整中文名稱",
  "stockCode": "${stockCode || '從財報自動辨識的代碼'}",
  "tickerSymbol": "純數字股票代碼，如 2330，不含 .TW，若無法辨識填 null",
